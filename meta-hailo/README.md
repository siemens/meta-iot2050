...
- `hailo-firmware` (proprietary firmware for the Hailo-8 chip)
- `hailort` (Userspace API), which includes:
  - `hailortcli` (command-line tool)
  - `hailort` (service)
  - `libhailort` (C library)
  - `libhailort-dev` (development package)
...
> cause `pyhailort` to fail.

## Supported Configurations

The following configurations are supported and recommended for achieving
optimal performance with the Hailo-8 AI Card:
...
You can find Hailo application examples at the
[Hailo Application Code Examples](https://github.com/hailo-ai/Hailo-Application-Code-Examples/tree/main/runtime)
repository, which includes examples for:
- C++
- GStreamer
- Python

For Python, it is strongly recommended to use a
...
> **Notes**:
> 1. Do not re-install a newer `numpy` package in the venv to overwrite the
...
>    with our current Hailo version. If you encounter any problems with the
>    latest examples, please try using an older version.
> 3. The examples provided are not officially supported by Hailo and are
>    provided solely on an “AS IS” basis and “with all faults”. They may not
>    offer an optimal experience on our platform. Please use them as
>    references to learn how to use Hailo and develop your own AI
>    applications.

Additionally, we have integrated three Tappas (GStreamer) demos to provide
users with a hands-on experience. For more details, please refer to the
[Tappas README](recipes-app/tappas/README.md):
 - Detection Pipeline
...
This layer is versioned according to the major Hailo driver version. Note that
the kernel ABI is not stable, and therefore the version of the userspace
components must perfectly match the version of the firmware and the kernel
module.

## Maintainers

See the `MAINTAINERS.md` file in this layer's directory.
