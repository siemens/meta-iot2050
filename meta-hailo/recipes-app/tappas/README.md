...
single video stream using an embedded host. The model needs an input 
video resolution of HD (720p). In the demo, it uses a 480p YUY2 camera 
input, which is then converted to HD (720p) in the GStreamer pipeline 
for the model.
...
located at `/usr/share/tappas/detection/resources/configs/yolov5.json`.
