...
# Send validated account actions to the root-owned Unix socket service.
# JSON is the default for automation; text mode remains available for humans.
...
USAGE = "Usage: iot2050-login-backend [--format text|json] <domain> <action> <user>"


def usage():
    print(USAGE, file=sys.stderr)
    print("Domains/actions:", file=sys.stderr)
    print("  failed-login status|reset", file=sys.stderr)
    print("  account status|set-password|unlock|disable|enable|delete", file=sys.stderr)
    print("Notes:", file=sys.stderr)
    print("  account set-password prompts for the new password twice.", file=sys.stderr)
    print("  JSON output is intended for automation; text output is operator-friendly.", file=sys.stderr)
    return 2
