...
if ! contains "${login_security_recipe}" 'cracklib-runtime'; then
    fail "Login security recipe must install the CrackLib runtime dictionary"
fi
pass "Login security recipe installs the CrackLib runtime dictionary"

if ! contains "${login_security_recipe}" 'wamerican'; then
    fail "Login security recipe must install an explicit CrackLib wordlist"
fi
pass "Login security recipe installs an explicit CrackLib wordlist"

if ! contains "${login_security_template}" 'CrackLib dictionary is required'; then
    fail "Login security template must fail clearly when the CrackLib dictionary is missing"
fi
pass "Login security template checks for the CrackLib dictionary"

if ! contains "${login_runtime_remote_checker}" 'R_CRACKLIB_DICT_EXISTS'; then
    fail "Remote runtime checker must verify the CrackLib dictionary"
fi
pass "Remote runtime checker verifies the CrackLib dictionary"
...
if ! contains "${login_security_template}" 'groupadd --system iot2050-admin'; then
    fail "Login security template must create iot2050-admin as a runtime fallback"
fi
pass "Login security template creates iot2050-admin as a runtime fallback"
...
if ! contains "${account_admin_helper}" 'Usage: $0 [--format text|json] <action> <user>' ||
    ! contains "${account_admin_helper}" 'Actions: set-password, unlock'; then
    fail "Account admin helper must document set-password and unlock actions"
fi
pass "Account admin helper documents set-password and unlock actions"
...
if ! contains "${failed_login_helper}" 'Usage: $0 [--format text|json] <action> <user>' ||
    ! contains "${failed_login_helper}" 'Actions: status, reset'; then
    fail "Failed-login helper must document status and reset actions"
fi
pass "Failed-login helper documents status and reset actions"
...
if ! contains "${login_admin_helper}" 'Usage: $0 [--format text|json] <command> [arguments]' ||
    ! contains "${login_admin_helper}" 'Commands: schema, schema-id, failed-login, account'; then
    fail "Unified login admin helper must document schema and action commands"
fi
pass "Unified login admin helper documents schema and action commands"
...
if ! contains "${login_backend_client}" 'Usage: iot2050-login-backend [--format text|json] <domain> <action> <user>' ||
    ! contains "${login_backend_client}" 'Domains/actions:' ||
    ! contains "${login_backend_client}" '  failed-login status|reset' ||
    ! contains "${login_backend_client}" '  account status|set-password|unlock|disable|enable|delete'; then
    fail "Login backend client must document the stable action interface"
fi
pass "Login backend client documents the stable action interface"
