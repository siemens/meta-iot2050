...
from iot2050_event_global import EVENT_IDENTIFIER

def write_event(event_type, event):
    if not event:
        raise EventIOError("Empty Events: no events to write")
    journal_stream = journal.stream(EVENT_IDENTIFIER)
...
def read_event(event_type):
    ...
    journal_reader.add_match("SYSLOG_IDENTIFIER={}".format(EVENT_IDENTIFIER))
